...
export function formatTaskRateCardUnitPrice(
  unitPrice: number,
  showWithRecharge = false,
  priceRate = 1,
  usdExchangeRate = 1,
  ...
  let priceInUSD = unitPrice * groupRatioMultiplier
  priceInUSD = applyRechargeRate(
    priceInUSD,
    showWithRecharge,
    priceRate,
...
export function formatTaskRateCardRange(
  model: PricingModel,
  showWithRecharge = false,
  priceRate = 1,
  usdExchangeRate = 1,
  ...
  const card = model.task_rate_card
  if (!card) return '-'
  ...
  const maxFormatted = formatTaskRateCardUnitPrice(
    maxPrice,
    showWithRecharge,
    priceRate,
    usdExchangeRate,
