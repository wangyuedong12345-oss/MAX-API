...
func testChannel(channel *model.Channel, testUserID int, testModel string, endpointType string, isStream bool) testResult {
	...
	if maxAPIError != nil {
		return testResult{
			context:     c,
			localErr:    maxAPIError,
			maxAPIError: maxAPIError,
		}
	}
	...
	if err != nil {
		return testResult{
			context:     c,
			localErr:    err,
			maxAPIError: types.NewError(err, types.ErrorCodeGenRelayInfoFailed),
		}
	}
	...
	if err != nil {
		return testResult{
			context:     c,
			localErr:    err,
			maxAPIError: types.NewError(err, types.ErrorCodeJsonMarshalFailed),
		}
	}
	...
	if err != nil {
		return testResult{
			context:     c,
			localErr:    err,
			maxAPIError: types.NewError(err, types.ErrorCodeChannelModelMappedError),
		}
	}
	...
	if info.RelayMode == relayconstant.RelayModeResponsesCompact &&
		...
		return testResult{
			...
			maxAPIError: types.NewError(fmt.Errorf("unsupported api type: %d", apiType), types.ErrorCodeInvalidApiType),
		}
	}
	...
	if adaptor == nil {
		return testResult{
			...
			maxAPIError: types.NewError(fmt.Errorf("invalid api type: %d, adaptor is nil", apiType), types.ErrorCodeInvalidApiType),
		}
	}
	...
	case relayconstant.RelayModeEmbeddings:
		...
		if embeddingReq, ok := request.(*dto.EmbeddingRequest); ok {
			...
			return testResult{
				...
				maxAPIError: types.NewError(errors.New("invalid embedding request type"), types.ErrorCodeConvertRequestFailed),
			}
		}
	case relayconstant.RelayModeImagesGenerations:
		...
		if imageReq, ok := request.(*dto.ImageRequest); ok {
			...
			return testResult{
				...
				maxAPIError: types.NewError(errors.New("invalid image request type"), types.ErrorCodeConvertRequestFailed),
			}
		}
	case relayconstant.RelayModeRerank:
		...
		if rerankReq, ok := request.(*dto.RerankRequest); ok {
			...
			return testResult{
				...
				maxAPIError: types.NewError(errors.New("invalid rerank request type"), types.ErrorCodeConvertRequestFailed),
			}
		}
	case relayconstant.RelayModeResponses:
		...
		if responseReq, ok := request.(*dto.OpenAIResponsesRequest); ok {
			...
			return testResult{
				...
				maxAPIError: types.NewError(errors.New("invalid response request type"), types.ErrorCodeConvertRequestFailed),
			}
		}
	...
	default:
		...
		if generalReq, ok := request.(*dto.GeneralOpenAIRequest); ok {
			...
			return testResult{
				...
				maxAPIError: types.NewError(errors.New("invalid general request type"), types.ErrorCodeConvertRequestFailed),
			}
		}
	}
	...
	if err != nil {
		return testResult{
			context:     c,
			localErr:    err,
			maxAPIError: types.NewError(err, types.ErrorCodeConvertRequestFailed),
		}
	}
		...
		if err != nil {
			if fixedErr, ok := relaycommon.AsParamOverrideReturnError(err); ok {
				return testResult{
					context:     c,
					localErr:    fixedErr,
					maxAPIError: relaycommon.MaxAPIErrorFromParamOverride(fixedErr),
				}
			}
			return testResult{
				context:     c,
				localErr:    err,
				maxAPIError: types.NewError(err, types.ErrorCodeChannelParamOverrideInvalid),
			}
		}
	}
	...
	if err != nil {
		return testResult{
			context:     c,
			localErr:    err,
			maxAPIError: types.NewOpenAIError(err, types.ErrorCodeDoRequestFailed, http.StatusInternalServerError),
		}
	}
	...
	if usageErr != nil {
		return testResult{
			context:     c,
			localErr:    usageErr,
			maxAPIError: types.NewOpenAIError(usageErr, types.ErrorCodeBadResponseBody, http.StatusInternalServerError),
		}
	}
	result := w.Result()
...
func TestChannel(c *gin.Context) {
	...
	if result.localErr != nil {
		...
		if result.maxAPIError != nil {
			resp["error_code"] = result.maxAPIError.GetErrorCode()
		}
	...
	if result.maxAPIError != nil {
		c.JSON(http.StatusOK, gin.H{
			"success":    false,
			"message":    result.maxAPIError.Error(),
			"time":       consumedTime,
			"error_code": result.maxAPIError.GetErrorCode(),
		})
		return
	}
	c.JSON(http.StatusOK, gin.H{
...
func testChannels(channels []*model.Channel, testUserID int, notify bool, allowDisable bool) error {
	...
	gopool.Go(func() {
		...
		for _, channel := range channels {
			...
			maxAPIError := result.maxAPIError
			// request error disables the channel
			if maxAPIError != nil {
				shouldBanChannel = service.ShouldDisableChannel(result.maxAPIError)
			}
			...
			if common.AutomaticDisableChannelEnabled && !shouldBanChannel {
				if milliseconds > disableThreshold {
					...
					maxAPIError = types.NewOpenAIError(err, types.ErrorCodeChannelResponseTimeExceeded, http.StatusRequestTimeout)
