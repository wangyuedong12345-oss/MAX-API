			...
			if shouldSelectChannel {
				...
				if strings.HasPrefix(c.Request.URL.Path, "/pg/chat/completions") {
					...
					if playgroundRequest.Group != "" {
						...
						if !service.CanUseTokenGroup(userGroup, playgroundRequest.Group) && playgroundRequest.Group != usingGroup {
...
func SetupContextForSelectedChannel(c *gin.Context, channel *model.Channel, modelName string) *types.MaxAPIError {
	...
	key, index, maxAPIError := channel.GetNextEnabledKey()
	if maxAPIError != nil {
		return maxAPIError
	}
